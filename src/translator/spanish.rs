...
impl SpanishTranslator {
    ...
    fn translate_fuzzy(&self, time: &TimeInfo, use_24h: bool, include_units: bool) -> String {
        ...
        match time.minute {
            ...
            15 => format!("{}{} y cuarto{}", Self::hour_number(hour), hour_unit_str, period),
            30 => format!("{}{} y media{}", Self::hour_number(hour), hour_unit_str, period),
            45 => {
                ...
                format!("cuarto para {}{}{}", Self::hour_number(next_hour), next_unit_str, period)
            },
            1..=7 => format!("{}{} y {}{}", Self::hour_number(hour), hour_unit_str, Self::minute_number(time.minute), period),
            8..=14 => format!("casi {}{} y cuarto{}", Self::hour_number(hour), hour_unit_str, period),
            16..=22 => format!("{}{} y veinte{}", Self::hour_number(hour), hour_unit_str, period),
            23..=29 => format!("casi {}{} y media{}", Self::hour_number(hour), hour_unit_str, period),
            31..=37 => format!("pasando {}{} y media{}", Self::hour_number(hour), hour_unit_str, period),
            38..=44 => {
                let next_hour = if use_24h {
                    if time.hour24 == 23 { 0 } else { time.hour24 + 1 }
                } else {
                ...
                let next_unit_str = if include_units { format!(" {}", Self::hour_unit(next_hour)) } else { String::new() };
                format!("casi cuarto para {}{}{}", Self::hour_number(next_hour), next_unit_str, period)
            },
            46..=52 => {
                let next_hour = if use_24h {
                    if time.hour24 == 23 { 0 } else { time.hour24 + 1 }
                ...
                let next_unit_str = if include_units { format!(" {}", Self::hour_unit(next_hour)) } else { String::new() };
                format!("casi {}{}{}", Self::hour_number(next_hour), next_unit_str, period)
            },
            _ => {
                let next_hour = if use_24h {
                    if time.hour24 == 23 { 0 } else { time.hour24 + 1 }
    ...
    fn translate_very_fuzzy(&self, time: &TimeInfo, use_24h: bool, include_units: bool) -> String {
        ...
        match time.minute {
            ...
            38..=52 => {
                ...
                let next_unit_str = if include_units { format!(" {}", Self::hour_unit(next_hour)) } else { String::new() };
                format!("casi cuarto para {}{}", Self::hour_number(next_hour), next_unit_str)
            },
            _ => {
                let next_hour = if use_24h {
                    if time.hour24 == 23 { 0 } else { time.hour24 + 1 }
